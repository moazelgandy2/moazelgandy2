# Moaz El Gandy

> **Software Engineer**
>
> Full Stack Engineer focused on building high-performance SaaS applications, cloud infrastructure, and distributed systems.
>
> [Portfolio](https://moazelgandy-portfolio.vercel.app) • [LinkedIn](https://www.linkedin.com/in/moazelgandy2) • [Email](mailto:moazelgandy@icloud.com)
>

---

## 🚀 Featured Engineering

| Project | Role | Technical Complexity |
| :--- | :--- | :--- |
| **[Droplo](https://github.com/moazelgandy2/droplo-core)** | **Core Developer** | **Custom PaaS Engine.** A reverse-engineered Vercel clone capable of containerizing Next.js apps on demand using Docker sockets, Hono streams, and Linux namespaces. |
| **[Kashkool](https://kashkool.online)** | **Full-stack Engineer** | **Multi-Tenant SaaS.** An educational platform featuring dynamic RBAC (Row Level Security), sub-domain isolation via middleware, and real-time state synchronization via Convex. |
| **Golden Waste** | **Frontend Engineer** | **Enterprise Logistics.** A live logistics platform managing complex asset lifecycles. Implemented state machines for asset tracking and role-based views (Admin/Driver/Operator). |

---

## 🛠 Technical Stack

**Core Languages**
`TypeScript` `JavaScript` `Go (Golang)` `Python` `Bash`

**Infrastructure & DevOps**
`Docker` `Linux (Ubuntu/Debian)` `Nginx` `GitHub Actions` `AWS` `Cloudflare (Workers/R2/DNS)`

**Backend Systems**
`Node.js` `Hono` `PostgreSQL` `Redis` `Convex` `Prisma`

**Frontend Engineering**
`Next.js (App Router)` `React` `Tailwind CSS` `Shadcn/UI`

---

## 📊 Technical Activity

<p align="left">
  <img src="https://wakatime.com/share/@moazelgandy2/8de696a2-2f88-49a8-8971-e70b2dece4c3.svg" alt="WakaTime Stats" />
</p>
